...
struct smokeBombPass : public FunctionPass {
  ...
  bool hasSmokeBombGlobals(Module *M);
  ...
  bool insertDummyCallInstBeforeInsn(Instruction *I);
  bool insertDummyCallInstAfterInsn(Instruction *I);
  bool insertOrInstBeforeInsn(Instruction *I);
  bool insertOrInstAfterInsn(Instruction *I);
...
bool smokeBombPass::insertOrInstBeforeInsn(Instruction *I) {
  Instruction * newInst = BinaryOperator::Create(Instruction::Or, I->getOperand(0), I->getOperand(0));
  I->getParent()->getInstList().insert(I->getIterator(), newInst);

  return true;
}


bool smokeBombPass::insertOrInstAfterInsn(Instruction *I) {
  Instruction * newInst = BinaryOperator::Create(Instruction::Or, I->getOperand(0), I->getOperand(0));
  I->getParent()->getInstList().insertAfter(I->getIterator(), newInst);

  return true;
}

bool smokeBombPass::insertDummyCallInstBeforeInsn(Instruction *I) {
  // CallInst *CI = NULL;
...
/*
  ...
  builder.CreateCall(SmokeBombDummyInitFunc);
...
/*
  ...
  builder.CreateCall(SmokeBombDummyExitFunc);*/
...
bool smokeBombPass::doInitialization(Module &M) {
  setSmokeBombFunc(&M);
  smokeBombGlobalFlag = hasSmokeBombGlobals(&M);
  return false;
}
...
bool smokeBombPass::hasSmokeBombGlobals(Module *M) {
  /* If the Module doesn't has any smoke-bomb-annotated global variable, skip checking for fast build */
  for (Module::global_iterator I = M->global_begin(), E = M->global_end(); I != E; ++I) {
    if (I->getName() == "llvm.global.annotations") {
      ConstantArray *CA = dyn_cast<ConstantArray>(I->getOperand(0));
      for(auto OI = CA->op_begin(); OI != CA->op_end(); ++OI){
        ...
        GlobalVariable *AnnotationGV = dyn_cast<GlobalVariable>(CS->getOperand(0)->getOperand(0));
        StringRef annotation = dyn_cast<ConstantDataArray>(AnnotationGL->getInitializer())->getAsCString();
        if (annotation.compare("smokeBomb") == 0) {
          return true;
        }
      }
    }
      ...
      if (GetElementPtrInst *PI = dyn_cast<GetElementPtrInst>(I)) {
        ...
        if (V->getValueID() == Value::GlobalVariableVal) {
          ...
          if (flag == true) {
            ...
            insertDummyCallInstBeforeInsn(I);
            insertDummyCallInstAfterInsn(I);
            // insertOrInstBeforeInsn(I);
            // insertOrInstAfterInsn(I);
            continue;
          }
        }
      }
